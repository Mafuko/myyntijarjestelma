...
- **Available everywhere `LocaleToggle` currently is** — matches locale's exact scope. **Correction from an earlier draft of this spec**: `LocaleToggle` does NOT actually appear on `/signup` or `/(auth)/invite/[token]` (verified by reading both files directly while writing the implementation plan — neither imports it) — it only appears on `/login`'s page and the dashboard layout (`app/(dashboard)/layout.tsx`). "Matches locale's exact scope" therefore means those same two places, not a broader pre-login footprint invented from the phrase "pre-login" alone. The cookie-based mechanism would work identically on signup/invite if ever added there, but adding it to pages locale itself skips would be new scope beyond "match the existing precedent," not a continuation of it.
