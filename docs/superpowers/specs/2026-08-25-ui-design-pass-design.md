...
- **Dependencies added:** shadcn/ui CLI-scaffolded primitives for the component inventory below. Of that set, only `Label` (`@radix-ui/react-label`) and `Separator` (`@radix-ui/react-separator`) pull in Radix; `Button`, `Input`, `Card`, `Table`, `Badge`, `Alert` are plain styled HTML with no Radix dependency. Plus `class-variance-authority`, `clsx`, `tailwind-merge` (shared by all shadcn components) and `lucide-react` (icons). No new UI framework, no CSS-in-JS — everything renders through Tailwind v4, which the project already uses.
...
- **Component inventory:** Button, Input, Label, Card, Table, Badge, Alert, Separator. No shadcn `Select`, `Checkbox`, `Dialog`, or `AlertDialog` — see constraints below.
...
- Playwright E2E specs (`tests/e2e/*.spec.ts`) and RTL component tests select primarily by role, label, placeholder, and visible text — not CSS classes. The shadcn primitives used (`Button`, `Input`, `Card`, `Table`, `Badge`, `Alert`, `Label`, `Separator`) all render the same underlying native element they replace (a shadcn `Button` is still a `<button>`), and `<select>`/checkbox stay native per the constraints above, so as long as visible text/labels/placeholders are kept unchanged, existing tests are expected to keep passing without rewrites.
