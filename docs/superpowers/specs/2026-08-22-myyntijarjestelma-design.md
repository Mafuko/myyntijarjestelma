...
- **Real-time:** SSE via a Route Handler streaming sale events to subscribed clients. A sale mutation (Server Action) writes to Postgres, then publishes to an in-process event emitter the SSE handler subscribes to. Works within Vercel's per-request model since each client holds one long-lived SSE connection.
- **File generation:**
