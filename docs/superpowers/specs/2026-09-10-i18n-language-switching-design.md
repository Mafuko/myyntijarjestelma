...
- **Phased delivery — this spec covers all three phases at the design level, but only Phase 1 gets a full implementation plan now:**
  ...
  - **Phase 2 (separate plan, later):** every remaining page, form, and Zod validation message (items, checkout — both the Scan and Sell-by-seller tabs, sales dashboard, members, admin, audit, event creation/edit).
