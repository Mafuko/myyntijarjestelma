...
6. **Mystery string "kaunista koodia" appearing in an error — source not found in this repo**
   - Reported alongside the STAFF price-tag access bug (see "Done since MVP" below — now fixed). Searched the full repository source tree, including `node_modules`, for `kaunista koodia` (case-insensitive) — no match anywhere.
   - Not yet resolved: this text doesn't originate from anything in this codebase as far as an exhaustive source search can tell, so it's likely coming from somewhere outside our control (a browser extension, a translation/grammar tool active in the reporter's browser, or something in the exact deployed environment rather than source). Needs a screenshot or exact reproduction steps (which browser, which page, is a translation extension active) to trace further.

7. **E2E suite is intermittently flaky in CI, root cause is running against `next dev`, not a production build**
