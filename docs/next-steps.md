...
5. **Price tags download immediately instead of showing a preview first**
...
6. **STAFF gets "You do not have access to this event" trying to view price tags, despite being an event member**
   - Root cause found: `lib/services/price-tags.tsx:45` gates price-tag generation to `requireEventAccess(session, eventId, ['SELLER', 'ADMIN'])` — `'STAFF'` is missing from the allowed-roles list, so any STAFF member is rejected regardless of membership status. Same class of bug as the earlier "My items" STAFF-redirect fix (`67f744b`): a role array that doesn't include every role that should legitimately reach the feature.
   - Fix is a one-line change (`['SELLER', 'STAFF', 'ADMIN']`) once it's decided STAFF should be able to view/print any seller's price tags (plausible, since STAFF runs checkout and may need to reprint a lost tag) — not yet implemented, tracked here per the request that reported it.

7. **Mystery string "kaunista koodia" appearing in an error — source not found in this repo**
   - Reported alongside item 6's access error. Searched the full repository source tree for `kaunista koodia` (case-insensitive) — no match anywhere in the app code, specs, or plans.
   - Not yet resolved: this text doesn't originate from anything in this codebase as far as a full source search can tell, so it's likely coming from somewhere outside our control (a browser extension, a translation/grammar tool active in the reporter's browser, or something in the exact deployed environment rather than source). Needs a screenshot or exact reproduction steps (which browser, which page, is a translation extension active) to trace further.

8. **E2E suite is intermittently flaky in CI, root cause is running against `next dev`, not a production build**
